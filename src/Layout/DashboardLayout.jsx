...
const DashboardLayout = () => {
  ...
  return (
    <div className="lg:flex lg:px-0 px-3 bg-white dark:bg-[#25293C] overflow-hidden h-screen w-full">
      {/* sideber  */}
      <div id="sideber" style={{ transition: ".5s" }}
        className={`max-w-80 pl-2 z-50 border-r dark:border-r-0 ${showText ? ` xl:w-[22%] lg:w-[23%] sm:w-[33%] w-[75%]  lg:relative` : `HideTextAnimation ${mouseEnterInSIderber ? 'xl:w-[19%] lg:w-[20%] sm:w-[33%] w-[75%]  box-border' : 'w-[64px]'} absolute  h-screen `} shadow-2xl min-h-screen absolute ${show ? 'left-0 top-0 SideberOpen' : 'hidden lg:block SideberClose'} bg-white dark:bg-[#25293C] dark:text-gray-100 z-50 `}
      >
        <Dashboard />
      </div>
      <div style={{ transition: "1s" }} className={`w-full h-screen pt-3 box-border ${showText ? `NavberwidhtAnimationClose` : 'lg:ml-16 NavberwidhtAnimationOpen '} lg:px-3 xl:px-6 px-3 px-auto box-border relative`}>
