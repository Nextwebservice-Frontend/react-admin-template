            ...
            <div className='px-4 '>
                {
                    LanguagePopUpOptions.map((item,index) =>
                        <button key={index} onClick={() => setshowLanguagePopUp(false)} className='w-full dark:text-gray-300 flex justify-start items-center gap-3 text-gray-500 text-base hover:text-gray-500 hover:no-underline hover:bg-slate-100 dark:hover:bg-slate-500 p-1 rounded-md transition-all'>
                            {item}
                        </button>)
                }
            </div>
