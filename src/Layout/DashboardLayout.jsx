...
const DashboardLayout = () => {
  ...
  return (
    <div className="lg:flex lg:px-0 px-[6px] bg-[#F8F7FA]  dark:bg-[#25293C] overflow-hidden w-full relative">
      ...
      <>
        <div id="sideber"
          className={`w-[260px] overflow-x-hidden  z-[70] border-r dark:border-r-0 ${showText ? `w-[260px] min-w-[260px] lg:relative` : `HideTextAnimation ${mouseEnterInSIderber ? 'transitionAnimationopen w-[260px]  box-border' : 'w-[58px] min-w-[58px] transitionAnimationclose'} absolute  h-screen `} shadow-2xl min-h-screen absolute ${show ? 'left-0 top-0 SideberOpen' : 'hidden lg:block SideberClose'} bg-white dark:bg-[#2F3349] dark:text-gray-100 z-50  ${semiDark ? 'semiDark dark' : ''}`}
        >
          <Dashboard />

        </div>
        {Content && <div className={`w-[58px] ${showText ? 'hidden' : 'hidden lg:block'}`}></div>}

      </>
      <div style={{ transition: "1s" }} className={`w-full h-screen  ${Content ? 'container mx-auto' : ''}   pt-3 box-border ${showText ? `NavberwidhtAnimationClose` : ` ${Content ? 'container ' : 'lg:ml-[58px] '} NavberwidhtAnimationOpen `} lg:px-3 xl:px-6 px-3 px-auto box-border relative`}>
