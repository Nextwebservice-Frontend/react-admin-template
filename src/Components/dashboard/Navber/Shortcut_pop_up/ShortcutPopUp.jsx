...
const ShortcutPopUp = ({ showSortcutPopUp, setshowSortcutPopUp }) => {
    ...
    return (
        <div  className={`absolute rounded-lg border lg:right-24 md:right-20 right-[2%] z-20 sm:-top-12 -top-8  overflow-hidden mt-10 bg-white ${showSortcutPopUp ? "popup" : "hidden"} sm:max-h-[90vh] overflow-y-hidden min-w-[96%] max-w-[96%] sm:max-w-80 sm:min-w-80`}>
            ...
            <div id="NotificationPopUp" className='grid grid-cols-2 gap-0 justify-center overflow-y-scroll items-center text-gray-500 mt-2 sm:max-h-[311px] max-h-[407px]'>
                {
                    ShortcutPopUpMenu.map((item, index) =>
                        <button onClick={() => setshowSortcutPopUp(false)} key={index} className='flex justify-center items-center font-semibold flex-col border sm:py-2 py-6 gap-1 tracking-[.8px] hover:bg-slate-100 transition-all text-sm'>
